...
export function WelcomeSection() {
  ...
  const plans = [{
    ...
    features: ['Até 5 questões por pesquisa', '100 respostas por pesquisa', '2 pesquisas por mês', 'Análise estatística básica']
  }, {
    id: 'vortex-neural',
    name: 'Vortex Neural',
    monthlyPrice: 649,
    ...
    isPopular: true,
    features: ['Até 10 questões por pesquisa', '250 respostas por pesquisa', '4 pesquisas por mês', 'Análise estatística intermediária', 'Análise de sentimentos segmentada']
  }, {
    id: 'nexus-infinito',
    name: 'Nexus Infinito',
    monthlyPrice: 1249,
    ...
    features: ['Questões ilimitadas por pesquisa', 'Respostas ilimitadas por pesquisa', '15 pesquisas por mês', 'Análise estatística avançada', 'Análise de sentimento multicanal', 'Modelos preditivos avançados']
  }];
  ...
  return <main className="min-h-screen bg-section-light">
      {/* Hero Section */}
      ...
      <section className="py-20 px-6 relative">
        <div className="max-w-6xl mx-auto">
            ...
            <div className="flex items-center justify-center mb-8">
              <div className="bg-white rounded-xl p-2 shadow-lg border border-gray-100">
                <div className="flex items-center gap-2">
                  <button onClick={() => setIsYearly(false)} className={`px-6 py-3 rounded-lg font-semibold transition-all duration-300 ${!isYearly ? 'bg-gradient-accent text-white shadow-md' : 'text-brand-gray hover:text-brand-dark-blue'}`}>
                    Mensal
                  </button>
                  <button onClick={() => setIsYearly(true)} className={`px-6 py-3 rounded-lg font-semibold transition-all duration-300 relative ${isYearly ? 'bg-gradient-accent text-white shadow-md' : 'text-brand-gray hover:text-brand-dark-blue'}`}>
                    Anual
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            return <div key={plan.name} className={`card-modern p-8 group relative ${plan.isPopular ? 'border-2 border-brand-green/30 bg-gradient-to-br from-white to-brand-green/5' : ''}`}>
                  ...
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <div className={`w-12 h-12 bg-red-600 rounded-xl flex items-center justify-center ${plan.isPopular ? 'shadow-[var(--shadow-glow)]' : ''}`}>
                  ...
                  <div className="mb-8">
                    ...
                    {isYearly && savings && <div className="text-sm text-brand-green font-semibold mb-2">
                  ...
                  <Button className={`w-full group-hover:shadow-[var(--shadow-elevated)] ${plan.isPopular ? 'btn-gradient glow-effect' : 'btn-hero'}`} onClick={() => handlePlanSelection(plan)}>
                    Comece Agora
                  </Button>
                </div>;
          })}
