...
BEGIN
    ...
    -- Iterate through the responses JSONB to find text answers (compatível com array e objeto)
    BEGIN
        IF jsonb_typeof(NEW.responses::jsonb) = 'array' THEN
            FOR question_data IN SELECT value FROM jsonb_array_elements(NEW.responses::jsonb) AS t(value)
            LOOP
                ...
                END IF;
            END LOOP;
        ELSIF jsonb_typeof(NEW.responses::jsonb) = 'object' THEN
            FOR question_data IN SELECT value FROM jsonb_each(NEW.responses::jsonb) AS t(key, value)
            LOOP
                response_text := question_data::text;
                response_text := trim(both '"' from response_text);
                IF length(response_text) > 10 AND response_text ~ '[a-zA-ZÀ-ÿ]' THEN
            ...
            END LOOP;
        ELSE
            -- Tipo inesperado, não processar
            RAISE NOTICE 'Sentiment analysis skipped: responses is not array or object';
        END IF;
    EXCEPTION
        WHEN OTHERS THEN
            -- Se houver erro ao processar responses, apenas retornar
    ...
    IF array_length(text_responses, 1) > 0 THEN
        BEGIN
            -- Call the Edge Function for sentiment analysis
            PERFORM
                net.http_post(
        ...
        EXCEPTION
            WHEN OTHERS THEN
                ...
                RAISE NOTICE 'Error calling sentiment analysis function: %', SQLERRM;
        END;
    END IF;
...
-- Comentários
COMMENT ON FUNCTION trigger_sentiment_analysis() IS 'Fixed version: Automatically triggers sentiment analysis for new responses with proper error handling (supports array and object)';
...
-- Log da correção
RAISE NOTICE 'Sentiment analysis trigger fixed to handle missing configurations, JSONB object/array, and prevent error 22023';