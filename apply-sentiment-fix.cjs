...
const supabaseServiceKey = process.env.VITE_SUPABASE_ANON_KEY; // Usando anon key pois service role não está no .env.local
...
async function applySentimentFix() {
  console.log('🔧 Aplicando correção do trigger de análise de sentimento...');
...
COMMENT ON TRIGGER sentiment_analysis_trigger ON responses IS 'Fixed version: Calls sentiment analysis Edge Function with error handling when new responses are inserted';
  ...
  try {
    ...
    console.log('✅ Correção do trigger aplicada com sucesso!');
    console.log('📝 O trigger agora possui tratamento de erro para configurações ausentes');
    return true;
...
applySentimentFix().then(success => {
  if (success) {
    console.log('🎉 Correção concluída! O erro 22023 deve estar resolvido.');
  } else {
    console.log('❌ Falha na aplicação da correção.');
  }
